...
const ButtonSection = ({ counter, setCounter, dislikedMovies, setDislikedMovies, likedMovies, setLikedMovies, movie }) => {
  ...
  return (
    <section className="voting-buttons">
      ...
      <button className="voting-buttons_btn dislike_btn" ref={dislike} onMouseDown={(e) => handleClick(e)}></button>
      {/* <img src={love} alt="heart icon" className="picker-img" /> */}
      <button className="voting-buttons_btn like_btn" ref={like} onMouseDown={(e) => handleClick(e)}></button>
    </section>
  )
}
